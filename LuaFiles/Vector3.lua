module("Vector3", package.seeall)
----------------------------------------------------------------------
-- @file Vector3.lua
-- a 3D Vector for experimenting with extention from C
-- @author Smoke and Mirrors Development
-- copyright 2010
-- @date 02/24/2010
--
require "ObjectOrientedParadigm"
require "Vector2"

local vector3ToString = function(self)
	return string.format("(%8.2f, %8.2f, %8.2f)", self.x or -111, self.y or -111, self.z or -111)
end


----------------------------------------------------------------------
Vector3.name = "Vector3"
-- Vector3.extends = "Vector2"


----------------------------------------------------------------------
-- adds a vector to self
-- @return self

function Vector3:add(v)
	self.x = self.x + v.x
	self.y = self.y + v.y 
	self.z = self.z + v.z
	return self
end

----------------------------------------------------------------------
-- adds a vector to self
-- @return self

function Vector3:addComponent(x, y, z)
	self.x = self.x + x
	self.y = self.y + y 
	self.z = self.z + z
	return self
end
----------------------------------------------------------------------
-- 3D vector component multiply
-- @return self

function Vector3:ComponentMul(v)
	self.x = self.x * v.x
	self.y = self.y * v.y
	self.z = self.z * v.z
	return self
end


----------------------------------------------------------------------
-- 3D vector component division
-- @return self

function Vector3:ComponentDiv(v)
	self.x = self.x / v.x
	self.y = self.y / v.y
	self.z = self.z / v.z
	return self
end

----------------------------------------------------------------------
-- 3D vector cross product
-- @return self

function Vector3:Cross(v)
	local a, b, c
	a = self.y * v.z - self.z * v.y
	b = self.z * v.x - self.x * v.z
	c = self.x * v.y - self.y * v.x
	self.x = a
	self.y = b
	self.z = c
	return self
end
----------------------------------------------------------------------
-- @return distance between to vectors

function Vector3:Distance(v)
	local element = self.x - v.x
	local length = (element * element)
	element = self.y - v.y
	length = length + (element * element)
	element = self.z - v.z
	length = length + (element * element)
	return math.sqrt(length)
end
----------------------------------------------------------------------
-- @return squared distance

function Vector3:DistanceSqr(v)
	local element = self.x - v.x
	local length = (element * element)
	element = self.y - v.y
	length = length + (element * element)
	element = self.z - v.z
	length = length + (element * element)
	return length
end
----------------------------------------------------------------------
-- @return distance in the xy plane only

function Vector3:DistanceXY(v)
	local length = 0
	local element = self.x - v.x
	length = length + (element * element)
	element = self.y - v.y
	length = length + (element * element)
	return math.sqrt(length)
end
----------------------------------------------------------------------
-- @return squared xy distance

function Vector3:DistanceXYSqr(v)
	local length = 0
	local element = self.x - v.x
	length = length + (element * element)
	element = self.y - v.y
	length = length + (element * element)
	return length
end
----------------------------------------------------------------------
-- @return vector dot product

function Vector3:Dot(v)
	return self.x * v.x + self.y * v.y + self.z * v.z
end
----------------------------------------------------------------------
-- unpack the elements of the class
-- @return 3 numbers

function Vector3:Elements()
	return self.x, self.y, self.z
end 
----------------------------------------------------------------------
-- internal vector compare. use == to compare pointers
-- @return true if the vectors have the same 3D elements

function Vector3:Equals(v)
	return (rawequal(self,v)) or (self.x == v.x and self.y == v.y and self.z == v.z)
end
----------------------------------------------------------------------
-- creates a table of 3 numbers

function Vector3:construct(...)
	local a, b, c = ...
	if a then
		if b then
			self.x = a
			self.y = b
			self.z = c or 0
		else
			self.x = a.x
			self.y = a.y
			self.z = a.z
		end
	else
		self.x = 0
		self.y = 0
		self.z = 0
	end
end
----------------------------------------------------------------------
-- checks wither the vector is close enough to another

function Vector3:IsFar(pos, range)
	return self:DistanceSqr(pos) >= (range * range)
end
----------------------------------------------------------------------
-- checks wither the vector is close enough to another

function Vector3:IsFarXY(pos, range)
	return self:DistanceXYSqr(pos) >= (range * range)
end
----------------------------------------------------------------------
-- checks wither the vector is close enough to another

function Vector3:IsNear(pos, range)
	return self:DistanceSqr(pos) <= (range * range)
end
----------------------------------------------------------------------
-- checks wither the vector is close enough to another
function Vector3:IsNearXY(pos, range)
	return self:DistanceXYSqr(pos) <= (range * range)
end
----------------------------------------------------------------------
-- checks each element for a valid number
function Vector3:IsValid()
	return math.isvalid(self.x) and math.isvalid(self.y) and math.isvalid(self.z)
end	
----------------------------------------------------------------------
-- @return the magnitude of the vector
function Vector3:Length()
	return math.sqrt(self.x * self.x + self.y * self.y + self.z * self.z)
end
----------------------------------------------------------------------
-- @return the squared magnitude of the vector
function Vector3:LengthSqr()
	return self.x * self.x + self.y * self.y + self.z * self.z 
end
----------------------------------------------------------------------
-- @return the magnitude of the vector on the xy plane only
function Vector3:LengthXY()
	return math.sqrt(self.x * self.x + self.y * self.y)
end
----------------------------------------------------------------------
-- @return the squared magnitude of the vector on the xy plane only 
function Vector3:LengthXYSqr()
	return self.x * self.x + self.y * self.y
end
----------------------------------------------------------------------
-- negates the vector
-- @return self
function Vector3:Negate()
	self.x = -self.x
	self.y = -self.y
	self.z = -self.z
	return self
end
----------------------------------------------------------------------
-- normalized the vector, sets it to unit length
-- @returns the original length of the vector, for convenience
function Vector3:Normalize()
	local length = math.sqrt(self.x * self.x + self.y * self.y + self.z * self.z)
	self.x = self.x / length
	self.y = self.y / length
	self.z = self.z / length
	return length
end

function Vector3:Rotate(m)
	self:Set(self.x * m[1][1] + self.y * m[2][1] + self.z * m[3][1],
		self.x * m[1][2] + self.y * m[2][2] + self.z * m[3][2],
		self.x * m[1][3] + self.y * m[2][3] + self.z * m[3][3])
	return self
end

----------------------------------------------------------------------
-- multiply each element of the vector by the passed in scalar
-- @param scalar multiple
-- @return self
function Vector3:Scale(scalar)
	self.x = self.x * scalar
	self.y = self.y * scalar
	self.z = self.z * scalar
	return self
end
----------------------------------------------------------------------
-- sets the vector to the passed in values
-- @param ... either a table with x,y,z indices (another vector), or 3 numbers
function Vector3:SetNoAssert(...)
	local x, y, z = ...		
	if y then
		self.x = x
		self.y = y 
		self.z = z
	elseif x then
		self.x = x.x
		self.y = x.y
		self.z = x.z
	end
	return self
end	

function Vector3:SetWithAssert(...)
	local x, y, z = ...		
	if y then
		assert(x and y and z, "bad data in Vector3")
		self.x = x
		self.y = y 
		self.z = z
	elseif x then
		assert(x.x and x.y and x.z, "bad data in Vector3")
		self.x = x.x
		self.y = x.y
		self.z = x.z
	else
		error("bad data in Vector3")
	end
	return self
end

if DEBUG_INTERPRETATION then
	Vector3.Set = Vector3.SetWithAssert
else
	Vector3.Set = Vector3.SetNoAssert
end
----------------------------------------------------------------------
-- subtracts a vector from self
-- @return self
function Vector3:Subtract(v)
	self.x = self.x - v.x
	self.y = self.y - v.y
	self.z = self.z - v.z
	return self
end

function Vector3:Transform(m)
	self:Set(self.x * m[1][1] + self.y * m[2][1] + self.z * m[3][1] + m[4][1],
		self.x * m[1][2] + self.y * m[2][2] + self.z * m[3][2] + m[4][2],
		self.x * m[1][3] + self.y * m[2][3] + self.z * m[3][3] + m[4][3])
	return self
end

function Vector3:WithinBounds(v1, v2)
	return self.x >= v1.x and self.x <= v2.x and self.y >= v1.y and self.y <= v2.y and self.z >= v1.z and self.z <= v2.z
end

----------------------------------------------------------------------
-- @return the magnitude of the vector
function Vector3:Distance(v)
	return math.sqrt((self.x - v.x) * (self.x - v.x) + (self.y - v.y) * (self.y - v.y) + (self.z - v.z) * (self.z - v.z))
end
----------------------------------------------------------------------
-- @return the squared magnitude of the vector
function Vector3:DistanceSqr(v)
	return (self.x - v.x) * (self.x - v.x) + (self.y - v.y) * (self.y - v.y) + (self.z - v.z) * (self.z - v.z)
end
----------------------------------------------------------------------
-- @return a string representation of the vector
Vector3.ToString = vector3ToString
----------------------------------------------------------------------
-- the "+" operator.  Adds a vector to self
-- @return self
function Vector3.__add(lhs, rhs)
	return new("Vector3", lhs.x + rhs.x, lhs.y + rhs.y, lhs.z + rhs.z)
end
----------------------------------------------------------------------
-- the ".." operator.  concats a string version
Vector3.__concat = ObjectOrientedParadigm.toStringConcat
----------------------------------------------------------------------
-- the "/" operator.  Devides each element of self by the divisor
-- @return self
function Vector3.__div(lhs, rhs)
	return new("Vector3", lhs.x / rhs, lhs.y / rhs, lhs.z / rhs)
end
----------------------------------------------------------------------
-- the "*" operator. multiplies each element of self by the scalar
-- @return self
function Vector3.__mul(lhs, rhs)
	return new("Vector3", lhs.x * rhs, lhs.y * rhs, lhs.z * rhs)
end
----------------------------------------------------------------------
-- the "-" (subtraction) operator.  Subtracts a vector from self
-- @return self
function Vector3.__sub(lhs, rhs)
	return new("Vector3", lhs.x - rhs.x, lhs.y - rhs.y, lhs.z - rhs.z)
end
----------------------------------------------------------------------
-- called when passed as an arguement to print(). 
-- @return a pretty, text version
Vector3.__tostring = vector3ToString
----------------------------------------------------------------------
-- the "-" (unary minus) operator.  Negates the vector
-- @return self
function Vector3.__unm(rhs)
	return new("Vector3", -rhs.x, -rhs.y, -rhs.z)
end
----------------------------------------------------------------------
-- the "==" compare by value operator
-- @return boolean
function Vector3.__eq(lhs, rhs)
	return rawequal(lhs, rhs) or (lhs.x == rhs.x and lhs.y == rhs.y and lhs.z == rhs.z)
end

declareClass(Vector3)

--[[ restore or remove the need for
-- to be used in internal calculations only
v3Scratch = new 'Vector3'
-- global scratch
_G.v3 = new 'Vector3' 
-- world origin
_G.origin = new('Vector3', 0,0,0)
-- global cardinal axes 
_G.WorldAxes = 
{
	posZ = 	new("Vector3", 0, 0, 1),
	negZ = 	new("Vector3", 0, 0,-1),
	posX = 	new("Vector3", 1, 0, 0),
	negX = 	new("Vector3",-1, 0, 0),
	posY = 	new("Vector3", 0, 1, 0),
	negY = 	new("Vector3", 0,-1, 0),
}
--]]
