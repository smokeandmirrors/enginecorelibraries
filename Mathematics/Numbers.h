...
inline dreal generateRandom(dreal min, dreal max) 
{
	const dreal v = ((dreal)generateRandom((sint)0, (sint)RAND_MAX)) * ( 1.0 / (dreal)RAND_MAX); 
	return v * (max - min) + min;
}

inline schar generateRandom(schar min, schar max) 
{
	return static_cast<schar>((rand() % (max - min))) + min;
}

inline sint generateRandom(sint min, sint max) 
{
	return (rand() % (max - min)) + min;
}

inline sreal generateRandom(sreal min, sreal max) 
{
	const sreal v = ((sreal)generateRandom((sint)0, (sint)RAND_MAX)) * ( 1.0f / (sreal)RAND_MAX); 
	return v * (max - min) + min;
}
